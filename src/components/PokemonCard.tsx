...
export function PokemonCard(props: { id: string; className?: string }) {
  ...
  return (
    <div
      className={cn(
        "flex h-72 w-full flex-col items-center justify-center rounded-lg p-4 shadow-inner shadow-white/50",
        props.className,
      )}
      style={{ backgroundColor: getPokeColor(data.types.at(0)?.type.name) }}
    >
      <h2 className="text-lg uppercase text-white">{data.name}</h2>
      <Image
      ...
      <div className="flex flex-row gap-4">
        {data.types.map((type) => (
          <span
            key={type.type.name}
            className="rounded-xl border px-2 py-1 uppercase shadow-inner shadow-white/50"
